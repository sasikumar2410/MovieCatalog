...
public class MovieCatalogResource {
	...
	public List<CatalogItems> getCatalog(@PathVariable("userId") String userId) {
		...
		return ratingsBag.stream()                                                  //putting the list of items [emptying the bag on a] conveyor belt
				.map(rating -> new CatalogItems(rating.getMovieId(), "description of "+ rating.getMovieId(), rating.getRating()))   //converting each rating item moving on the conveyor belt TO  catalogitem
